...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        for (int i = 0;; i++)
        {
            ...
            pinocchio::Data::MatrixXs Jt;
            pinocchio::Data::Matrix6 JJt;
            Jt = J.transpose();
            JJt.noalias() = J * Jt;
            double Alpha = -err.dot(JJt * err) / ((JJt * err).dot(JJt * err));
            // Update the q with delta_q = alpha*Jt*err
            q = pinocchio::integrate(model, q, Alpha * Jt * err);
